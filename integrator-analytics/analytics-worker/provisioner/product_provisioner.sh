...
nohup ${WORKING_DIRECTORY}/${WSO2_SERVER}-${WSO2_SERVER_VERSION}/wso2/analytics/wso2/worker/bin/carbon.sh &

sleep 10

# tail the WSO2 product server startup logs until the server startup confirmation is logged
tail -f ${WORKING_DIRECTORY}/${WSO2_SERVER}-${WSO2_SERVER_VERSION}/wso2/analytics/wso2/worker/logs/carbon.log | while read LOG_LINE
do
  # echo each log line
  echo "${LOG_LINE}"
